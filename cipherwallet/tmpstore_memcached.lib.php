...
function get_mcd_handle() {
// connect to the memcached servers, if not already connected
// the list of servers is defined in the constants file
	global $mcdservers;
	$mcd = new Memcached("mcd");
	if (count($mcd->getServerList()) == 0) {
		$mcd->setOption(Memcached::OPT_COMPRESSION, FALSE);
		$mcd->setOption(Memcached::OPT_LIBKETAMA_COMPATIBLE, TRUE);
		$mcd->addServers($mcdservers);
	}
	return $mcd;
}
...
// failure means that a nonce with the same key already exists
	$mcd = get_mcd_handle();
	return $mcd->add(sprintf(K_NONCE, $arg1, $arg2), 0, $ttl);
}

function cw_session($session_id, $var, $value=NULL) {
// session variables managed in the temp store
	$mcd = get_mcd_handle();
    $s = json_decode($mcd->get(sprintf(K_CW_SESSION, $session_id)), TRUE);
    if (is_null($value)) {
        ...
        $s[$var] = $value;
        return $mcd->set(sprintf(K_CW_SESSION, $session_id), json_encode($s), CW_SESSION_TIMEOUT) ? $value : NULL;
    }
}
...
//    polling mechanism
	$mcd = get_mcd_handle();
	return $mcd->set(sprintf(K_USER_DATA, $session_id), json_encode($user_data), 30) ? $session_id : "";
}
...
//    a QR code
	$mcd = get_mcd_handle();
	return $mcd->get(sprintf(K_USER_DATA, $session_id));
}
...
// it returns a new login credentials record
	...
	return ($mcd->set(sprintf(K_SIGNUP_REG, $session_id), json_encode($creds), $complete)) ? 
...
//    confirmation tag that we saved with the function above
	$mcd = get_mcd_handle();
	if ($creds_json = $mcd->get(sprintf(K_SIGNUP_REG, $session_id)))
...
//    functions on the login page
	$mcd = get_mcd_handle();
	return $mcd->set(sprintf(K_USER_IDENT, $session_id), $user_ident, 30) ? $session_id : "";
}
...
//    to retrieve user identification data posted with the function above
	$mcd = get_mcd_handle();
	return $mcd->get(sprintf(K_USER_IDENT, $session_id));
}
